...
/datum/shape/Destroy(force, ...)
	for(var/datum/vector2d/victim in base_points)
		qdel(victim)
	for(var/datum/vector2d/victim in rel_points)
		qdel(victim)
	for(var/datum/vector2d/victim in normals)
		qdel(victim)
	. = ..()
...
/datum/shape/proc/set_points(list/points)
	for(var/datum/die in rel_points)
		qdel(die)
	for(var/datum/die in normals)
		qdel(die)
...
/datum/shape/proc/_recalc()
	if(!src.rel_points || !src.rel_points.len)
		return
	if(!src.base_points)
		message_admins("[src] has no base points")
	for(var/i in 1 to src.base_points.len)
		var/datum/vector2d/rel_point = src.base_points[i]
		rel_point.rotate(src._angle)
		src.rel_points[i] = rel_point

	//Clear out our current AABB collision box
	for(var/datum/vector2d/foo in aabb)
		qdel(foo)
	src.aabb.Cut()
	var/min_x = INFINITY
	var/min_y = INFINITY
...
/datum/shape/proc/collides(var/datum/shape/other)
	...
	for (var/datum/vector2d/norm in src.normals)
		if(SSphysics_processing.is_separating_axis(src.position, other.position, src.rel_points, other.rel_points, norm, c_response))
			qdel(c_response)
			return FALSE
	...
	if (c_response)
		...
		c_response.overlap_vector.multiply(c_response.overlap)
...
/datum/shape/proc/get_collision_point(var/datum/shape/other)
	...
	var/list/src_points = src.get_global_points()
	var/list/other_points = other.get_global_points()

	var/list/collision_points = list()

	for (var/src_i = 1, src_i < src_points.len, src_i++)
		for (var/other_i = 1, other_i < other_points.len, other_i++)
			var/datum/vector2d/intersection = closest_point.get_seg_intersect(src_points[src_i], src_points[src_i+1], other_points[other_i], other_points[other_i+1])
			if(intersection)
	...
	for (var/datum/vector2d/collision_point in collision_points)
		closest_point.add(collision_point)

	closest_point.update(closest_point.x / collision_points.len, closest_point.y / collision_points.len)
